...
with st.sidebar:
    ...
    if phase == "setup":
        ...
        set_session("resume_text", resume_text)

        st.markdown("---")
        max_q = st.slider("Number of Questions", min_value=3, max_value=7, value=5)
        set_session("max_questions", max_q)
        ...
        if st.button("🚀 Start Interview", use_container_width=True):
            with st.spinner("Preparing your interview..."):
                ...
                first_q = generate_question(role, resume_context_for_prompt(get_session("parsed_resume")), [])
        ...
        st.progress(q_asked / max_q, text=f"{q_asked}/{max_q} answered")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    feedbacks = get_session("feedbacks")
    current_question = get_session("current_question")
    max_q = get_session("max_questions")
    ...
    if submit and answer_input.strip():
        ...
        if len(answers) >= max_q:
            ...
            with st.spinner("Preparing next question..."):
                # Optionally generate a follow-up or advance
    ...
    if answers:
        ...
        for i, (q, a, ev) in enumerate(zip(questions[:-1], answers, feedbacks), 1):
            composite = answer_composite_score(ev)
            color = score_color(composite)
            with st.expander(f"Q{i} — Score: {composite:.0f}/100 — {q[:60]}..."):
